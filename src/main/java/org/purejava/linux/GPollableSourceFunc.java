...
public interface GPollableSourceFunc {
    ...
    static MemorySegment allocate(GPollableSourceFunc fi, SegmentScope scope) {
        return RuntimeHelper.upcallStub(constants$472.GPollableSourceFunc_UP$MH, fi, constants$472.GPollableSourceFunc$FUNC, scope);
    }
    static GPollableSourceFunc ofAddress(MemorySegment addr, SegmentScope scope) {
        MemorySegment symbol = MemorySegment.ofAddress(addr.address(), 0, scope);
        return (java.lang.foreign.MemorySegment _filter_info, java.lang.foreign.MemorySegment _user_data) -> {
            try {
                return (int)constants$472.GPollableSourceFunc_DOWN$MH.invokeExact(symbol, _filter_info, _user_data);
            } catch (Throwable ex$) {
