...
public interface __sighandler_t {
    ...
    static MemorySegment allocate(__sighandler_t fi, SegmentScope scope) {
        return RuntimeHelper.upcallStub(constants$73.__sighandler_t_UP$MH, fi, constants$73.__sighandler_t$FUNC, scope);
    }
    static __sighandler_t ofAddress(MemorySegment addr, SegmentScope scope) {
        MemorySegment symbol = MemorySegment.ofAddress(addr.address(), 0, scope);
        return (int __x0) -> {
            try {
                constants$73.__sighandler_t_DOWN$MH.invokeExact(symbol, __x0);
            } catch (Throwable ex$) {
