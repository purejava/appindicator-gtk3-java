// Generated by jextract

package org.purejava.appindicator;

import java.lang.foreign.Arena;
import java.lang.foreign.MemoryLayout;
import java.lang.foreign.MemorySegment;
import java.lang.foreign.SegmentAllocator;
import java.lang.invoke.VarHandle;
/**
 * {@snippet :
 * struct _GParamSpecChar {
 *     struct _GParamSpec parent_instance;
 *     signed char minimum;
 *     signed char maximum;
 *     signed char default_value;
 * };
 * }
 */
public class _GParamSpecChar {

    public static MemoryLayout $LAYOUT() {
        return constants$657.const$1;
    }
    public static MemorySegment parent_instance$slice(MemorySegment seg) {
        return seg.asSlice(0, 72);
    }
    public static VarHandle minimum$VH() {
        return constants$657.const$2;
    }
    /**
     * Getter for field:
     * {@snippet :
     * signed char minimum;
     * }
     */
    public static byte minimum$get(MemorySegment seg) {
        return (byte)constants$657.const$2.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * signed char minimum;
     * }
     */
    public static void minimum$set(MemorySegment seg, byte x) {
        constants$657.const$2.set(seg, x);
    }
    public static byte minimum$get(MemorySegment seg, long index) {
        return (byte)constants$657.const$2.get(seg.asSlice(index*sizeof()));
    }
    public static void minimum$set(MemorySegment seg, long index, byte x) {
        constants$657.const$2.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle maximum$VH() {
        return constants$657.const$3;
    }
    /**
     * Getter for field:
     * {@snippet :
     * signed char maximum;
     * }
     */
    public static byte maximum$get(MemorySegment seg) {
        return (byte)constants$657.const$3.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * signed char maximum;
     * }
     */
    public static void maximum$set(MemorySegment seg, byte x) {
        constants$657.const$3.set(seg, x);
    }
    public static byte maximum$get(MemorySegment seg, long index) {
        return (byte)constants$657.const$3.get(seg.asSlice(index*sizeof()));
    }
    public static void maximum$set(MemorySegment seg, long index, byte x) {
        constants$657.const$3.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle default_value$VH() {
        return constants$657.const$4;
    }
    /**
     * Getter for field:
     * {@snippet :
     * signed char default_value;
     * }
     */
    public static byte default_value$get(MemorySegment seg) {
        return (byte)constants$657.const$4.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * signed char default_value;
     * }
     */
    public static void default_value$set(MemorySegment seg, byte x) {
        constants$657.const$4.set(seg, x);
    }
    public static byte default_value$get(MemorySegment seg, long index) {
        return (byte)constants$657.const$4.get(seg.asSlice(index*sizeof()));
    }
    public static void default_value$set(MemorySegment seg, long index, byte x) {
        constants$657.const$4.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena scope) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, scope); }
}


